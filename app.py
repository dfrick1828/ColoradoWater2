...
# -----------------------------
# Decree and well permit search helpers
# -----------------------------
def normalize_decree_case_for_cdss(case_number):
    s = str(case_number or "").strip().upper()
    s = re.sub(r"[^0-9A-Z]", "", s)
...
def normalize_decree_case_for_laserfiche(case_number):
    s = str(case_number or "").strip().upper()
    s = re.sub(r"[^0-9A-Z]", "", s)
    m = re.match(r"^(?:20)?(\d{2})CW0*(\d+)$", s)
    if m:
...
def cdss_decree_transaction_url(case_number, division):
    ...
    return (
        "https://dwr.state.co.us/Rest/GET/api/v2/waterrights/transaction"
        f"?caseNumber={quote_plus(case)}&division={int(division)}&format=json&pageSize=500000"
    )


@st.cache_data(ttl=60 * 60, show_spinner=False)
def fetch_decree_transactions_direct(case_number, division):
    case = normalize_decree_case_for_cdss(case_number)
    params = {
        "caseNumber": case,
...
def normalize_well_permit_number(permit_number):
    return str(permit_number or "").strip().upper()


def cdss_well_permit_rest_url(permit_number="", receipt="", county=""):
    ...
    if str(receipt).strip():
        params.append(f"receipt={quote_plus(str(receipt).strip())}")
    if str(county).strip():
        params.append(f"county={quote_plus(str(county).strip())}")
    return "https://dwr.state.co.us/Rest/GET/api/v2/wellpermits/wellpermit?" + "&".join(params)


def cdss_well_permit_action_rest_url(permit_number="", receipt=""):
    params = ["format=json", "pageSize=500000"]
    permit = normalize_well_permit_number(permit_number)
    if permit:
        params.append(f"permitNumber={quote_plus(permit)}")
...
@st.cache_data(ttl=60 * 60, show_spinner=False)
def fetch_well_permits_direct(permit_number="", receipt="", county=""):
    params = {"format": "json", "pageSize": "500000"}
    permit = normalize_well_permit_number(permit_number)
    if permit:
        params["permitNumber"] = permit
    if str(receipt).strip():
        params["receipt"] = str(receipt).strip()
    if str(county).strip():
        params["county"] = str(county).strip()
    headers = {}
    try:
        api_key = st.secrets.get("CDSS_API_KEY", "")
        if api_key:
...
@st.cache_data(ttl=60 * 60, show_spinner=False)
def fetch_well_permit_actions_direct(permit_number="", receipt=""):
    params = {"format": "json", "pageSize": "500000"}
    permit = normalize_well_permit_number(permit_number)
    if permit:
        params["permitNumber"] = permit
...
with search_tab_decree:
    st.caption("Search decree metadata by case number and Water Division. API links are under Advanced details.")
    ...
    with dc3:
        st.write("")
        st.write("")
        run_decree_search = st.button(
            "Search Decree",
            use_container_width=True,
            key="run_decree_search_clean",
        )
    ...
    if run_decree_search:
        try:
            ...
            if decree_tx_clean.empty:
                ...
                st.success(
                    f"Found {len(decree_tx_clean):,} transaction record(s) for {cdss_case_clean} "
                    f"in Division {decree_division_clean}."
                )
...
with search_tab_well:
    st.caption("Search CDSS well permits. API links are under Advanced details.")
    ...
    with w4:
        st.write("")
        st.write("")
        run_well_search = st.button(
            "Search Permit",
            use_container_width=True,
            key="run_well_search_clean",
        )
        ...
        if not (permit_norm_clean or receipt_input_clean.strip() or county_input_clean.strip()):
            ...
            try:
                ...
                if permits_df_clean.empty:
                    ...
                    st.dataframe(
                        permits_df_clean[cols] if cols else permits_df_clean,
                        use_container_width=True,
                        height=260,
                    )

                actions_df_clean = pd.DataFrame()
                action_url_clean = cdss_well_permit_action_rest_url(
                    permit_norm_clean,
                    receipt_input_clean,
                )
                try:
                ...
                if not actions_df_clean.empty:
                    with st.expander("Permit action history"):
                        ...
                        st.dataframe(
                            actions_df_clean[action_cols] if action_cols else actions_df_clean,
                            use_container_width=True,
                            height=260,
                        )
                ...
                with st.expander("Advanced / API details"):
                    ...
                    st.code(
                        cdss_well_permit_action_rest_url(
                            permit_norm_clean,
                            receipt_input_clean,
                        )
                    )
